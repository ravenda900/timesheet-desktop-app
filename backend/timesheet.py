...
handler = RotatingFileHandler('timesheet_entry.log', maxBytes=100000000,
...
class Timesheet:
    def __init__(self, encryption_manager, api):
        self.encryption_manager = encryption_manager
        self.api = api

    @set_interval(1)
    def scheduled_run(self, callback):
        while True:
            while True:
            ...
            if self.mode == 'Automatic':
                schedule.every().day.at(self.scheduled_time).do(self.create_timesheet)
            elif self.mode == 'Manual':
                schedule.every().day.at(self.scheduled_time).do(callback)
    ...
    def create_timesheet(self):
        ...
        self.get_missing_timesheet_entries(date)
    ...
    def get_missing_timesheet_entries(self, date):
        ...
        self.missing_entries = missing_timesheet_entries
    ...
    def get_myself(self, email=None, password=None):
